...
namespace Ddon.Repositiry.Dapper
{
    public class DapperRepository : IDapperRepository
    {
        ...
        private string BuildWhereKeySqlByTable<T>(bool isIDs = false)
        {
            ...
            return $"SELECT {string.Join(",", alias)} FROM {tableName}(NOLOCK) WHERE {pk.FirstOrDefault().Key} {(isIDs ? "IN" : "=")} @Key";
        }

        private string BuildSelectSql<T>()
        {
            var tableName = GetTableName<T>();
            var fields = ReadOnlyQueryExtensions.GetFields<T>();
            var alias = fields.Select(p => $"{p.Key} {p.Value}");
        ...
        private KeyValuePair<DynamicParameters, List<string>> BuildParams(object param)
        {
            var paramCount = 0;
        ...
        class ReadOnlyQueryExtensions
        {
            private static readonly ConcurrentDictionary<Type, Dictionary<string, string>> _fields;

            private static readonly ConcurrentDictionary<Type, Dictionary<string, string>> _primaryKeyFields;

            static ReadOnlyQueryExtensions()
            {
                _fields = new ConcurrentDictionary<Type, Dictionary<string, string>>();
                _primaryKeyFields = new ConcurrentDictionary<Type, Dictionary<string, string>>();
            }

            public static Dictionary<string, string> GetFields<T>()
            {
                return _fields.GetOrAdd(typeof(T), _ =>
                {
            ...
            {
                return _primaryKeyFields.GetOrAdd(typeof(T), _ =>
                {
