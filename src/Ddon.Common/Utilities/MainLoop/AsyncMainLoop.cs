...
{
    /// <summary>
    /// 异步主循环：循环体为 <c>Func&lt;CancellationToken, Task&gt;</c>。
    /// </summary>
    /// <example>
    /// <code>
    ...
    ///     loopAction: async ct => await FetchDataAsync(ct));
    ///
    ...
    /// </code>
    /// </example>
    public sealed class AsyncMainLoop : MainLoopBase
    {
        private readonly Func<CancellationToken, Task> _loopAction;

        /// <param name="interval">循环间隔</param>
        /// <param name="loopAction">每次循环执行的异步委托</param>
        ...
        /// </param>
        public AsyncMainLoop(
            TimeSpan interval,
            Func<CancellationToken, Task> loopAction,
