...
namespace Ddon.VitrinPLC.TagEngine
{
    ...
    public sealed class ChangeNotifier : IChangeNotifier
    {
        ...
        public IDisposable Subscribe<T>(string tagName, Action<T> handler)
        {
            ...
            var sub = new Subscription(tagName, (_, newVal) =>
            {
                try { handler((T)Convert.ChangeType(newVal, typeof(T))); }
                catch { }
            }, this);
        ...
        public IDisposable Subscribe<T>(string tagName, Action<T, T> onChanged)
        {
            ...
            var sub = new Subscription(tagName, (oldVal, newVal) =>
            {
                try { onChanged((T)Convert.ChangeType(oldVal, typeof(T)), (T)Convert.ChangeType(newVal, typeof(T))); }
                catch { }
            }, this);
        ...
        public void NotifyChanges(IEnumerable<TagChange> changes)
        {
            foreach (var change in changes)
            {
                ...
                lock (_listLock)
                {
                    ...
                    {
                        if (sub.IsAlive)
                            sub.Invoke(change.OldValue, change.NewValue);
                        else
        ...
        public sealed class Subscription : IDisposable
        {
            ...
            public Subscription(string tagName, Action<object, object> handler, ChangeNotifier owner)
            {
                _tagName = tagName;
                _handler = handler;
                _owner = owner;
            ...
            {
                if (!_disposed) _handler(oldValue, newValue);
            }
