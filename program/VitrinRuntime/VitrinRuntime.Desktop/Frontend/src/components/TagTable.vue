<script setup lang="ts">
  ...
  const historyRecords = ref<TagHistoryRecord[]>([]);
  const historyLoading = ref(false);
  const historyTagName = ref('');

  async function handleShowHistory(tagName: string) {
    historyTagName.value = tagName;
  ...
  function getDefaultEditingValue(dataType: string): unknown {
  ...
  async function loadTags() {
    ...
    try {
      ...
      // 初始化新点位的修改值为默认值（不覆盖用户正在编辑的值）
      const currentIds = new Set(tags.value.map((t) => t.id));
      for (const tag of tags.value) {
        if (!(tag.id in editingValue.value)) {
          editingValue.value[tag.id] = getDefaultEditingValue(tag.dataType);
        }
      }
      // 清理已删除点位的编辑值
  ...
  <div style="height: 100%">
    ...
    <el-table v-loading="loading" :data="tags" border stripe size="small" max-height="500" style="width: 100%">
      ...
      <el-table-column label="修改值" width="200">
        <template #default="{ row }: { row: TagInfo }">
          <el-switch v-if="row.dataType === 'Bool'" v-model="editingValue[row.id]" size="small" active-text="ON" inactive-text="OFF" />
          <el-input-number v-else-if="['Int16', 'UInt16', 'Int32', 'UInt32'].includes(row.dataType)" v-model="editingValue[row.id]" size="small" controls-position="right" />
          <el-input-number v-else-if="['Float', 'Double'].includes(row.dataType)" v-model="editingValue[row.id]" size="small" :precision="row.dataType === 'Float' ? 4 : 8" controls-position="right" />
          <el-input v-else-if="row.dataType === 'String'" v-model="editingValue[row.id]" size="small" />
    ...
    <!-- 历史记录对话框 -->
    <el-dialog v-model="historyDialogVisible" :title="'历史记录 - ' + historyTagName" width="700px">
      <el-table v-loading="historyLoading" :data="historyRecords" border stripe size="small" max-height="500" style="width: 100%">
        <el-table-column label="时间" min-width="160">
          <template #default="{ row }: { row: TagHistoryRecord }">
        ...
        <el-table-column label="旧值" min-width="120">
          <template #default="{ row }: { row: TagHistoryRecord }">
            <el-tag v-if="row.dataType === 'Bool'" :type="row.oldValue ? 'success' : 'info'" size="small">
              {{ row.oldValue ? 'TRUE' : 'FALSE' }}
            </el-tag>
        ...
        <el-table-column label="新值" min-width="120">
          <template #default="{ row }: { row: TagHistoryRecord }">
            <el-tag v-if="row.dataType === 'Bool'" :type="row.newValue ? 'success' : 'info'" size="small">
              {{ row.newValue ? 'TRUE' : 'FALSE' }}
            </el-tag>
            <span v-else style="font-weight: 600; color: #409eff">{{ row.newValue ?? '-' }}</span>
          </template>
        </el-table-column>
      </el-table>
